...
const EXAM_SMART_SNAPSHOT_KEY = "studyplanner:exam-smart-snapshot";
...
/** Last generated exam-period smart planning (exams + révisions), scoped by logged-in user. */
export function saveExamSmartSnapshot(snapshot) {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.setItem(
      scopedKey(EXAM_SMART_SNAPSHOT_KEY),
      JSON.stringify({
        ...snapshot,
...
export function loadExamSmartSnapshot() {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(scopedKey(EXAM_SMART_SNAPSHOT_KEY));
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== "object") return null;
    return parsed;
...
export function clearExamSmartSnapshot() {
  if (typeof window === "undefined") return;
  try {
    window.localStorage.removeItem(scopedKey(EXAM_SMART_SNAPSHOT_KEY));
  } catch {
    // ignore
  }
}
...
export function backendPlanningToUiEvents(planning) {
  ...
  return events
    .map((ev) => {
      ...
      const normalizedType = normalizeTypeKey(ev?.type ?? ev?.sessionType);
      const type =
